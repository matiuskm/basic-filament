    ...
    <div class="flex flex-wrap flex-row">
        @foreach ($portfolios as $portfolio)
            ...
            <div class="relative overflow-hidden cursor-pointer mb-6">
                <a href="{{$portfolio->image}}" data-gallery="gallery1" data-glightbox="title: {{$portfolio->title}}; description: {{$portfolio->description}}" class="glightbox3">
                <img class="block w-full h-auto transform duration-500 grayscale hover:scale-125" src="{{$portfolio->image}}" alt="{{$portfolio->title}}">
                <div class="absolute inset-x-0 bottom-0 h-20 transition-opacity duration-500 ease-in opacity-0 group-hover:opacity-100 overflow-hidden px-4 py-2 text-gray-100 bg-black text-center">
                    ...
                    @php $firstWord = explode(' ', trim($portfolio->title))[0]; @endphp
                    <small class="d-block">{{ $firstWord }}</small>
                </div>
                </a>
            </div>
            </figure>
